...
NODE must be closer to us than the kth closest node in the routing table."
  ...
  (flet ((node-distance-from-us (node)
    ...
    (let* ((id (node-id node))
           ...
           (id-as-int (convert-id-to-int id))
           (our-id-as-int (convert-id-to-int *node-id*)))
      (and (<= lower-bound id-as-int upper-bound)
           (<= lower-bound our-id-as-int upper-bound)
           (< (calculate-distance id-as-int our-id-as-int)
