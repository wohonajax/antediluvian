...
(defun refresh-tokens ()
  ...
  (maphash (lambda (ip tokens-for-ip)
             (setf (gethash ip *token-ips*)
                   (remove-if-not #'valid-token-p tokens-for-ip)))
           *token-ips*)
  (maphash (lambda (info-hash tokens-for-hash)
             (mapc (lambda (token)
                     (unless (valid-token-p token)
                       (setf (gethash info-hash *token-hashes*)
                             (remove token tokens-for-hash :test #'equalp :count 1))
